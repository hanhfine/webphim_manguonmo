...
<div class="wrap cinema-admin-wrap">
	<h1><?php esc_html_e('Showtime Timeline', 'cinema-booking'); ?></h1>

	<form method="get" class="cinema-filter-bar">
		<input type="hidden" name="page" value="cinema-booking-timeline">
		<label>
			<?php esc_html_e('Date', 'cinema-booking'); ?>
			<input type="date" name="date" value="<?php echo esc_attr($selected_date); ?>">
		</label>
		<button type="submit" class="button button-primary"><?php esc_html_e('Filter', 'cinema-booking'); ?></button>
	</form>
		...
		<?php foreach ($rows as $row) : ?>
			<div class="cinema-timeline-row">
				<div class="cinema-timeline-room">
					<strong><?php echo esc_html($row['room']->post_title); ?></strong>
					<span><?php echo esc_html($row['cinema_label']); ?></span>
				</div>
				<div class="cinema-timeline-items">
					<?php if (empty($row['items'])) : ?>
						<div class="cinema-timeline-empty"><?php esc_html_e('No showtimes', 'cinema-booking'); ?></div>
					<?php endif; ?>
